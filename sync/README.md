...
1. EufyLife アプリで「ヘルスケア」への同期をオンにする
2. ショートカットアプリで新規ショートカットを作る
3. 「ヘルスケアのサンプルを検索」
   種類＝体重／並べ替え＝終了日／降順／上限 1 件
4. 「URL の内容を取得」
   - URL … `https://<あなたの Worker>.workers.dev/v1/inbox/weight`
   - 方法 … POST
   - ヘッダ … `Authorization` ＝ `Bearer <合鍵>`
   - 本文 … JSON、`kg` ＝（手順3の「値」）
5. オートメーションで「毎朝7時」か「EufyLife を閉じたとき」に動かす
   （「実行前に尋ねる」はオフに）

本文を組み立てるのが面倒なら、URL に付けるだけでも入ります。

```
https://<あなたの Worker>.workers.dev/v1/inbox/weight?kg=81.4&fat=28.2
```

「80.6 kg」のように単位が付いた文字列でも、「2026年9月12日 7:12」のような
日付でも読めます。日付を付けなければ、送った日のぶんになります。

アプリ側は「筋トレ」タブを開いたときに、そっと取り込みます。
手で入れたいときは「体重計から取り込む」→「いま取り込む」。

合鍵はショートカットのヘッダに入れるだけにして、URL には付けないで
ください（URL は記録に残ることがあります）。
