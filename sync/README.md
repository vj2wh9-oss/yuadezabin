...
トレーニングの日ごとに、**器具を使うか自重だけか**を決められます。

- **設定 → からだと目標** … 「トレーニングする曜日」と、そのうち
  「自重でやる曜日」を選びます。選ばなかった曜日はジムの器具を使います
- **計画自動作成 →「何でやる」** … その回だけ「ジムだけ」「自重だけ」に寄せられます
  （今週は出張、など）
- **その日の画面 →「自重に変える」** … 部位はそのままに、器具なしの種目へ
  その場で入れ替えます。計画を作り直さなくてよいので、
  「今日はジムに行けない」ときに使えます

自重の日も、**部位の回り方はジムの日と同じ**です（腕の日は腕の日のまま）。

### 回し方

「設定 → からだと目標」で決めた**トレーニングする曜日**の数で、分割が変わります。
...
- **自重の日は、器具を使う種目を落とす**。残ったものの道具は「自重」に言い直し、
  重さは 0 にする（マシン・バーベル・ダンベル・ケーブルなどは通さない）
- 休みの日は種目を入れない
