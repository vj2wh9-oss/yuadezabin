...
1. **EufyLife アプリ**で「ヘルスケア」への同期をオンにする
   （マイページ → 設定 → ヘルスケア連携。iOS の許可画面で「体重」の
   書き込みを許可する）
2. **ショートカット App** → 下の「ショートカット」タブ → 右上の **＋**
3. 検索欄に「ヘルスケア」と入れて **「ヘルスケアのサンプルを検索」** を足す
   - 「ヘルスケアのサンプル」を押して **体重** を選ぶ
   - **並べ替え** … 終了日／**降順**（新しい順）
   - **上限** … オンにして **1**
4. 検索欄に「テキスト」と入れて **「テキスト」** を足す
   - 送り先を貼り付ける
     `https://<あなたの Worker>.workers.dev/v1/inbox/weight?kg=`
   - そのうしろに、手順3の結果（変数）を入れる。
     入れた変数を押して **「値」** を選ぶ（数字だけになる）
5. 検索欄に「URL」と入れて **「URL の内容を取得」** を足す
   - **URL** … 手順4の「テキスト」を変数として入れる
   - 「詳しく表示」を開いて、**方法** を `POST` にする
   - 同じ「詳しく表示」の中の **ヘッダ** に1行足す
     キー `Authorization` ／ テキスト `Bearer <合鍵>`
     （`Bearer` と合鍵の間は半角スペース1つ）
6. 右上の **▶** で試す。`{"ok":true,"added":1}` が返れば通っている
   （返り値を見たいときは「クイックルック」を最後に足す）
7. **オートメーション**タブ → ＋ → 「毎日 7:00」または
   「App … EufyLife … 閉じたとき」→ 「ショートカットを実行」で
   作ったショートカットを選ぶ → **「実行前に尋ねる」をオフ**

初回の実行時に「ヘルスケアの読み出し」を聞かれるので許可します。

体脂肪も送るなら、手順4のうしろに `&fat=` と体脂肪の変数を足します。
日付を指定するなら `&date=2026-09-15`。付けなければ送った日のぶんです。

アプリ側は「筋トレ」タブを開いたときに、そっと取り込みます。
手で入れたいときは「体重計から取り込む」→「いま取り込む」。

合鍵はヘッダにだけ入れて、URL には付けないでください
（URL は記録に残ることがあります）。
...
- `{"error":"unauthorized"}` … ヘッダの綴り（`Authorization`）と
  `Bearer ` のうしろの空白、合鍵そのものを確かめる
- `{"error":"no_weight"}` … `?kg=` のうしろに数字が入っていない。
  手順4の変数が「値」になっているか（「サンプル」のままだと
  `81.2 kg` のような文になる）、手順3の上限が 1 になっているかを確かめる
